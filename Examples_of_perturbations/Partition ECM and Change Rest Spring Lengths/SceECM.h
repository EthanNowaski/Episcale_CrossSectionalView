...
//	SceNodes* nodes;
	...
	// Ethan's Edits New Function
	void AssignECMNodeStiffnessAndRestLength() ;
...
struct MechProp: public thrust::unary_function<EType,DD> {
    int* cellNeighborIdPtr;
    int* cellSubdomainIndxPtr;

	__host__ __device__ 
	MechProp(int* cellNeighborIdPtr, int* cellSubdomainIndxPtr)
        : cellNeighborIdPtr(cellNeighborIdPtr), cellSubdomainIndxPtr(cellSubdomainIndxPtr) {}

	__device__ 
	DD operator()(const thrust::tuple<EType, int>& t) const {
        EType nodeType = thrust::get<0>(t);
        int nodeIdx = thrust::get<1>(t);
		double stiffness ;
		double sponLen   ;    

		DefineECMStiffnessAndLknot(nodeType, nodeIdx, cellNeighborIdPtr, cellSubdomainIndxPtr, stiffness, sponLen); 
